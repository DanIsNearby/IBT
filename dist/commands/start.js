...
const lisk_framework_chain_connector_plugin_1 = require("@liskhq/lisk-framework-chain-connector-plugin");
...
const setPluginConfig = (config, flags) => {
    ...
    if (flags['dashboard-plugin-port'] !== undefined) {
        config.plugins[lisk_framework_dashboard_plugin_1.DashboardPlugin.name] = (_d = config.plugins[lisk_framework_dashboard_plugin_1.DashboardPlugin.name]) !== null && _d !== void 0 ? _d : {};
...
class StartCommand extends lisk_commander_1.BaseStartCommand {
    async getApplication(config) {
        ...
        if (flags['enable-chain-connector-plugin']) {
            app.registerPlugin(new lisk_framework_chain_connector_plugin_1.ChainConnectorPlugin(), { loadAsChildProcess: true });
        }
        return app;
    }
...
StartCommand.flags = {
    ...
    'enable-chain-connector-plugin': core_1.Flags.boolean({
        description: 'Enable ChainConnector Plugin. Environment variable "LISK_ENABLE_CHAIN_CONNECTOR_PLUGIN" can also be used.',
